...
METRICS_INGESTED = '_index=sumologic_volume _sourceCategory="sourcecategory_metrics_volume"| parse regex "\\"(?<sourcecategory>[^\\"]+)\\"\\:(?<data>\\{[^\\}]*\\})" multi| json field=data "dataPoints"| timeslice 1h | sum(datapoints) as datapoints by _timeslice, sourcecategory| datapoints*' + str(METRICS_CREDIT_RATE) + ' as credits'
